...
function PriorityFilterSection({
  ...
  priorityColumns: Record<string, string>;
  columnFilterConfigs: Record<string, ColumnFilter>;
  table: any;
  t: any;
}) {
...
export function FinancialDataTable() {
  ...
  const [priorityFilters, setPriorityFilters] = useState<Record<string, any>>({
    'sales_grow_per_year': null,
    'margin': null,
    'debt': null
  });
  ...
  const findMatchingColumns = useMemo(() => {
    ...
    const patterns = {
      sales_grow_per_year: /sales.*grow.*year|grow.*sales.*year|sales.*year.*grow|year.*sales.*grow/i,
      margin: /margin|profit.*margin|gross.*margin|net.*margin/i,
